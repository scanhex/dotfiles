...
pub async fn process_output(config: &Config, text: &str) -> Result<()> {
    ...
    match config.output {
        OutputType::Clipboard => write_to_clipboard(output_text),
        OutputType::Paste => direct_type_text(output_text),
        OutputType::File => {
            if let Some(ref path) = config.file {
                ...
                Err(anyhow!(
                    "Output type is file, but no output file specified."
                ))
            }
        }
        OutputType::Stdout => {
